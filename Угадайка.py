from random import*
word_list = ['свобода', 'равноправие', 'независимость', 'счастье', 'благосостояние', 'стабильность', 'коготь', 'король', 'север', 'принц', 'ночь', 'стена', 'одичалый', 'дракон', 'память', 'престол', 'год', 'человек', 'время', 'дело', 'жизнь', 'день', 'рука', 'раз', 'работа', 'слово', 'место', 'лицо',
...
def show_word(literal, word, print_word):
  for i in range(len(word)):
    if literal == word[i].lower():
      print_word = print_word[:i] + literal + print_word[i + 1:]
  return print_word
...
print('\nВот загаданное слово: ', print_word)
...
while True:
  if tries == 0:
    ...
    while True:
      literal = input('Введите букву: ').lower()
      if literal in guess_words:
        print('Эта буква уже была, повнимательнее.')
      elif literal not in alphabet:
        print('\nНе понял тебя... Давай попрубуем ещё разок.\n')
        print(print_word)
        print(display_hangman(tries))
      elif literal in alphabet and literal not in guess_words:
        break
    guess_words += literal
    
    if literal.upper() in word:
      print('Верно!\n')
      print_word = show_word(literal, word, print_word)
      
      if print_word == word.lower():
        print('\n          Победа!')
        print('Загаданное слово:', word)
        break
      print(print_word)
    elif literal.upper() not in word:
      print('Неверно\n')
      print(print_word)
      tries -= 1
    print(display_hangman(tries))
